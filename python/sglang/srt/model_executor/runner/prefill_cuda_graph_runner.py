...
class PrefillCudaGraphRunner(BaseCudaGraphRunner):
    ...
    def _prepare_forward_metadata_for_replay(
        ...
        if self._dcp_extend_active:
            # The gather and attention breaks read live indices and a persistent
            # bucket-sized buffer, because k_nope/k_pe are padded to that bucket.
            metadata = self._plan_dcp_metadata(forward_batch)
    ...
    def can_replay_locally(
        ...
        *,
        ...
        if self._has_uncapturable_chunked_prefix(prefix_lens):
            return False
        # Prefix hits need a cross-rank gather with per-batch shapes and stay eager.
